...
\begin{align*}
P(z, x|\alpha, \beta) =& \int d\theta \prod_{i=1}^K P(\theta|\alpha) \prod_{n=1}^{N} P(z_n |\theta) P(x_n|\beta_{z_n}) d\theta\\
=&  \prod_{n=1}^{N} P(x_n|\beta_{z_n})  \int d\theta \prod_{i=1}^K P(\theta|\alpha) \prod_{n=1}^{N} P(z_n |\theta) d\theta\\
...
Recall that we treated $\beta$ as constant, but usually in real world applications, we also don't know it. Since a$\beta_k$ is a parameter of a multinomial, we can model $\beta \sim Dirichlet(\gamma)$. In that case, the above sampling rules for $z_n$s do not change. Given everything, a posteriori $\beta_k$ is $\beta \sim Dirichlet(\gamma + C_k)$, where $C_k$ is a vector of counts over the entire data set (not for a single set as we write when we were deriving the sampling rules for $z_n$'s), $C_{k,v}$ being number of times $v\in \mathcal{X}$ is assigned to cluster $k$. 

The sampling procedure that we describe can be efficiently implemented in a MapReduce like environment. Sampling $z_n$'s, for each set, can be performed in parallel in the \textit{map} phase, and then we can sample from $\beta$ in the \textit{reduce} phase.
